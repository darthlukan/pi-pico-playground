...
#warning pio/hello_pio example requires a board with a regular LED
    ...
    // We use pio_claim_free_sm_and_add_program_for_gpio_range so we can address gpios >= 32 if
    ...
    // The state machine is now running. Any value we push to its TX FIFO will appear on the LED pin.
    // press a key to exit
